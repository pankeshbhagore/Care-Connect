...
const QUICK_ACTIONS = [
  ...
  {emoji:"🤕",label:"Headache",msg:"I have a severe headache"},
  {emoji:"📞",label:"Emergency Nos.",msg:"What are the emergency numbers in India"},
  {emoji:"💡",label:"Health Tip",msg:"Give me a health tip"},
];
...
function TypingDots() {
  return (
    <div style={{display:"flex",gap:4,padding:"12px 14px",background:"var(--bg-elevated)",border:"1px solid var(--border)",borderRadius:"4px 16px 16px 16px",width:"fit-content"}}>
      {[0,1,2].map(i=>(
        <span key={i} style={{width:7,height:7,borderRadius:"50%",background:"var(--accent)",display:"inline-block",
...
export default function AIChatbot({ userLocation, onRequestAmbulance, onShowHospitals }) {
  ...
  const [messages, setMessages]= useState([{
    id:0, role:"assistant", urgent:false,
  ...
  const sendMessage = useCallback(async(text)=>{
    ...
    const userMsg = { id:msgIdRef.current++, role:"user", text:text.trim(), time:new Date() };
    ...
    try {
      ...
      const aiMsg = {
        id:msgIdRef.current++,
        role:"assistant",
        text: d.response||"I couldn't process that. Please try again.",
      ...
      setMessages(p=>[...p,{
        id:msgIdRef.current++, role:"assistant", urgent:false, time:new Date(),
        text:"Sorry, I'm having trouble right now.\n\n**For emergencies: Call 108 or 112 immediately.**",
        suggestions:["Call 108 - Ambulance","Call 112 - Emergency"],
      }]);
    }
    setLoading(false);
  ...
  return(
    <>
      <style>{`
        ...
        .chat-suggestion:hover{background:var(--accent)!important;color:#fff!important;border-color:var(--accent)!important}
      `}</style>
      ...
      <button onClick={toggleOpen} style={{
        ...
        background:open?"var(--bg-elevated)":"linear-gradient(135deg,#0088cc,#0055aa)",
        border:open?"2px solid var(--border)":"none",
        boxShadow:open?"none":"0 4px 20px rgba(0,136,204,.45)",
        display:"flex",alignItems:"center",justifyContent:"center",
        fontSize:24,transition:"all .2s",color:open?"var(--text-muted)":"#fff",
        fontWeight:open?400:700,
      }}>
        {open?"✕":"💬"}
        {unread>0&&!open&&(
          <div style={{position:"absolute",top:-4,right:-4,background:"#ff4060",color:"#fff",width:20,height:20,
            borderRadius:"50%",fontSize:10,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",
      ...
      {open&&(
        <div style={{
          ...
          height:580,maxHeight:"calc(100vh - 112px)",
          background:"var(--bg-card)",border:"1px solid var(--border)",
          borderRadius:20,boxShadow:"0 10px 48px rgba(0,0,0,.25)",
          display:"flex",flexDirection:"column",overflow:"hidden",
          animation:"chatFadeIn .2s ease-out",
        }}>
          ...
          <div style={{background:"linear-gradient(135deg,#0055aa,#0088cc)",padding:"14px 16px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:10,color:"rgba(255,255,255,.7)",display:"flex",alignItems:"center",gap:4}}>
                <span style={{width:6,height:6,background:"#00e676",borderRadius:"50%",display:"inline-block",boxShadow:"0 0 5px #00e676"}}/>
          ...
          <div style={{borderBottom:"1px solid var(--border)",padding:"8px 10px",overflowX:"auto",display:"flex",gap:5,flexShrink:0}}>
            {QUICK_ACTIONS.map(qa=>(
              <button key={qa.label} onClick={()=>sendMessage(qa.msg)} style={{
                ...
                whiteSpace:"nowrap",flexShrink:0,transition:"all .15s",display:"flex",alignItems:"center",gap:4,
              }}
          ...
          <div style={{flex:1,overflowY:"auto",padding:"12px",display:"flex",flexDirection:"column",gap:10}}>
            {messages.map(msg=>(
              <div key={msg.id} className="chat-msg" style={{display:"flex",flexDirection:msg.role==="user"?"row-reverse":"row",gap:8,alignItems:"flex-start"}}>
                {msg.role==="assistant"&&(
                  <div style={{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#0088cc,#0055aa)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,flexShrink:0,marginTop:2}}>🤖</div>
                )}
                <div style={{maxWidth:"80%"}}>
                  <div style={{
                    ...
                    background:msg.role==="user"
                      ...
                      :msg.urgent?"rgba(255,64,96,.08)":"var(--bg-elevated)",
                    border:msg.urgent?"1px solid rgba(255,64,96,.35)":msg.role==="user"?"none":"1px solid var(--border)",
                    color:msg.role==="user"?"#fff":"var(--text-primary)",
                    fontSize:13,lineHeight:1.65,
                  }}>
                    {msg.urgent&&(
                      <div style={{fontSize:11,fontWeight:700,color:"#ff4060",marginBottom:5,display:"flex",alignItems:"center",gap:4}}>
                  ...
                  {msg.suggestions?.length>0&&(
                    <div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:6}}>
                      {msg.suggestions.map(sug=>(
                        <button key={sug} className="chat-suggestion" onClick={()=>handleSuggestion(sug)} style={{
            ...
            {loading&&(
              <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                <div style={{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#0088cc,#0055aa)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>🤖</div>
                <TypingDots/>
              </div>
            )}
            <div ref={msgEndRef}/>
          ...
          <div style={{padding:"10px 12px",borderTop:"1px solid var(--border)",background:"var(--bg-card)",flexShrink:0}}>
            <form onSubmit={handleSubmit} style={{display:"flex",gap:8}}>
              <input
                ...
                placeholder="Ask about health, hospitals, medicines…"
                disabled={loading}
                style={{flex:1,padding:"9px 14px",border:"1px solid var(--border)",borderRadius:20,
              ...
              <button type="submit" disabled={loading||!input.trim()} style={{
                width:38,height:38,borderRadius:"50%",flexShrink:0,cursor:input.trim()?"pointer":"default",
                background:input.trim()?"linear-gradient(135deg,#0088cc,#0055aa)":"var(--bg-elevated)",
                border:input.trim()?"none":"1px solid var(--border)",
                color:input.trim()?"#fff":"var(--text-muted)",
                display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,transition:"all .2s",
              }}>➤</button>
            </form>
            <div style={{textAlign:"center",fontSize:9,color:"var(--text-dim)",marginTop:5}}>
              For life-threatening emergencies call 108 · AI is not a substitute for professional medical advice
            </div>
          </div>
        </div>
      )}
